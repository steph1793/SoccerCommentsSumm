...
class DecoderLayer(tf.keras.layers.Layer):
	def __init__(self, d_model, num_heads, dff, rate=0.1, use_stats=False):
    ...
    if use_stats:
      ...
      self.stat_fc2 = tf.keras.layers.Dense(d_model, activation=tf.keras.activations.relu)


	def call(self, x, enc_output, training, look_ahead_mask, padding_mask, stats=None):
    ...
    if self.use_stats:
      ...
      out2 = tf.concat([ tf.expand_dims(stat_embed, 1),  out2], axis=-1)
