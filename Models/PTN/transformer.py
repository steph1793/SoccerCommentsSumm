...
class Decoder(tf.keras.layers.Layer):
    ...
    if use_stats:
      ...
      self.dec_layers += [DecoderLayer(d_model, num_heads, dff, rate, use_stats) ]
    else:
      self.dec_layers = [DecoderLayer(d_model, num_heads, dff, rate) 
							  for _ in range(num_layers)]
		self.dropout = tf.keras.layers.Dropout(rate)
	...
	def call(self, embed_x, enc_output, training, look_ahead_mask, padding_mask, stats=None):
    ...
    #################### stats part ##################
    out, block1, block2 = self.dec_layers[self.num_layers-1](out, enc_output, training,
													look_ahead_mask, padding_maskn, stats)
		
    attention_weights['decoder_layer{}_block1'.format(self.num_layers)] = block1
    attention_weights['decoder_layer{}_block2'.format(self.num_layers)] = block2
...
class Transformer(tf.keras.Model):
	...
	def call(self, inp, extended_inp,max_oov_len, tar, training, enc_padding_mask, look_ahead_mask, dec_padding_mask, stats=None):
		...
		dec_output, attention_weights, p_gens = self.decoder(embed_dec, enc_output, training, look_ahead_mask, dec_padding_mask, stats)
