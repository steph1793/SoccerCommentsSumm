...
def example_generator(filenames, vocab, max_enc_len, max_dec_len, mode, batch_size):
  ...
  for raw_record in parsed_dataset:
    ...
    statistics = raw_record["statistics"].numpy()
    
    start_decoding = vocab.word_to_id(vocab.START_DECODING)
    stop_decoding = vocab.word_to_id(vocab.STOP_DECODING)
    
    #article_sents = nltk.tokenize.sent_tokenize(article) ############### first sentence
    #article_words = article_sents[0].split()[ : max_enc_len]
    ...
    output = {
        ...
        "target" : target,
        "dec_len" : dec_len,
        "article" : article, #article_sents[0], ################ first sentence
        "abstract" : abstract,
