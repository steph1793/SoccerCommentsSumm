...
def beam_decode(model, batch, vocab, params):
  ...
  def decode_onestep(batch, enc_outputs, dec_state, dec_input, cov_vec):
    """
    ...
    res = model(enc_outputs, dec_state,batch[0]["enc_input"], batch[0]["extended_enc_input"], dec_input, batch[0]["max_oov_len"], cov_vec, batch[0]["stats"])
    final_dists, dec_hidden, context_vector, attentions, p_gens = res["final_dists"], res["dec_hidden"], res["context"], res["attn_weights"], res["p_gens"]
  ...
  class Hypothesis:
    ...
    def __init__(self, tokens, log_probs, state, attn_dists, p_gens, cov_vec):
    ...
    def extend(self, token, log_prob, state, attn_dist, p_gen, cov_vec):
  ...
  hyps = [Hypothesis(tokens=[vocab.word_to_id('[START]')], # we initalize all the beam_size hypothesises with the token start
                    ...
                    cov_vec = tf.zeros([enc_outputs.shape[1], 1])) for _ in range(params['batch_size'])] # batch_size == beam_size
  ...
  while steps < params['max_dec_steps'] and len(results) < params['beam_size'] : 
    ...
    if params['coverage']:
      cov_vec =  tf.stack([h.cov_vec for h in hyps], axis=0)
    else:
      cov_vec = None

    # we decode the top likely 2 x beam_size tokens tokens at time step t for each hypothesis
    returns = decode_onestep( batch, enc_outputs, tf.stack(states, axis=0), tf.expand_dims(latest_tokens, axis=1), cov_vec)
    ...
    for i in range(num_orig_hyps):
      h, new_state, attn_dist, p_gen, new_coverage_i = hyps[i], new_states[i], attn_dists[i], p_gens[i], returns["cov_vec"][i] if type(returns["cov_vec"]) != type(None) else None
  ...
  oovs = [oov.decode() for oov in batch[0]["article_oovs"][0].numpy()]
  best_hyp.abstract = " ".join(Data_Helper.output_to_words(best_hyp.tokens, vocab, oovs)[1:-1])
  best_hyp.text = batch[0]["article"].numpy()[0].decode()
  a = batch[1]["abstract"].numpy()[0].decode()
  if a :
    best_hyp.real_abstract = a
  return best_hyp