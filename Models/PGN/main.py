...
import argparse
from train_test_eval import train, restore_and_train, test_and_save, evaluate, evaluate_all
import os
...
def main():
  ...
  parser.add_argument("--cov_weight", default=0.5, help="Max number of examples to evaluate", type=float)
  parser.add_argument("--mode", help="training, eval or test options", default="", type=str)
  parser.add_argument("--from_model", help="training from a model", default="", type=str2bool)
  parser.add_argument("--coverage", help="add coverage or not", default="", type=str2bool)
  ...
  parser.add_argument("--new_checkpoint_dir", help="New Checkpoint directory when training from a specific model", default="", type=str)
  parser.add_argument("--test_save_dir", help="Directory in which we store the decoding results", default="", type=str)
  parser.add_argument("--eval_save_dir", help="Directory in which we store the evaluation results", default="", type=str)
  ...
  parser.add_argument("--use_stats", help="use statistics or not", default="", type=str2bool)
  ...
  if params["from_model"]:
    assert  params["model_path"], "You must specify a model path"
  ...
  if params["mode"] == "train":
    if params["from_model"]:
      restore_and_train(params)
      return
    train( params)    
  elif params["mode"] == "test":
    test_and_save(params)
  elif params["mode"] == "eval":
    if params["model_path"].lower() == "all":
      evaluate_all(params)
    else:
      evaluate(params)
